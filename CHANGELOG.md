...
* `hampel/sparkpost` 0.4.0 changes bounce classification: `BounceClassification` gains an
...
* `guzzlehttp/psr7` is required at `^2.0|^3.0`. `GuzzleHttp\Psr7\HttpFactory`, the PSR-17 fallback
  used when the container has nothing bound, does not exist before `guzzlehttp/psr7` 2.0
